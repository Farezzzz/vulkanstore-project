...
@section('content')
    ...
    <dialog id="modal_tambah" class="modal">
        <div class="modal-box rounded-[8px] bg-white p-6 shadow-lg">
            ...
            <form action="{{ route('pemasok.store') }}" method="POST" onsubmit="return validasiForm(event, 'modal_tambah')">
                @csrf

                <div class="mb-4">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">NAMA PEMASOK</label>
                    <input type="text" name="Nama_Pemasok" placeholder="Contoh: PT. Logistik Jaya" class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
                </div>

                <div class="mb-4">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">KONTAK PEMASOK</label>
                    <input type="text" name="Kontak_Pemasok" placeholder="Contoh: 0812-xxxx-xxxx" class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
                </div>

                <div class="mb-6">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">KATEGORI PEMASOK</label>
                    <select name="Kategori_Pemasok" class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
    ...
    <dialog id="modal_edit" class="modal">
        <div class="modal-box rounded-[8px] bg-white p-6 shadow-lg">
            ...
            <form id="form_edit" action="" method="POST" onsubmit="return validasiForm(event, 'modal_edit')">
                ...
                <div class="mb-4">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">NAMA PEMASOK</label>
                    <input type="text" id="edit_nama" name="Nama_Pemasok" placeholder="Ubah nama pemasok..." class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
                </div>

                <div class="mb-4">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">KONTAK PEMASOK</label>
                    <input type="text" id="edit_kontak" name="Kontak_Pemasok" placeholder="Ubah kontak pemasok..." class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
                </div>

                <div class="mb-6">
                    <label class="mb-2 block text-[12px] font-semibold text-[#091426]">KATEGORI PEMASOK</label>
                    <select id="edit_kategori" name="Kategori_Pemasok" class="h-[41px] w-full rounded border border-slate-300 px-4 text-sm text-[#091426] outline-none focus:border-[#091426] focus:ring-1 focus:ring-[#091426]">
                        <option value="INTERNAL (DIVISI VULKANISIR)">Internal (Divisi Vulkanisir)</option>
                        <option value="EXTERNAL">External</option>
                    </select>
                </div>
    ...
    <script>
        function validasiForm(event, modalId) {
            ...
            const nama = form.elements['Nama_Pemasok'].value.trim();
            const kontak = form.elements['Kontak_Pemasok'].value.trim();
            const kategori = form.elements['Kategori_Pemasok'].value.trim();

            if (!nama || !kontak || !kategori) {
                event.preventDefault();
                Swal.fire({
                    icon: 'warning',
                    title: 'Data Belum Lengkap!',
                    text: 'Pastikan Nama, Kontak, dan Kategori sudah terisi semua!',
            ...
            const regexHP = /^08\d{8,11}$/;
                if (!regexHP.test(kontak)) {
                    event.preventDefault();
                    Swal.fire({
                        icon: 'error',
                        title: 'Format Kontak Tidak Sesuai!',
                        text: 'Nomor HP wajib diawali "08", khusus angka, dan panjangnya 10-13 digit!',
                        confirmButtonColor: '#d33',
                        target: document.getElementById(modalId)
                    });
                    form.elements['Kontak_Pemasok'].focus();
                    return false;
                }
            return true;
        }
